...
3.  **Describe what this data frame contains.**
    ...
    -   This data frame contains one column called "state" with the USA state abbreviations and another column called "pop" with the total population of each state, with a total of 51 rows.
...
20. **During week 2, we discuss [Choosing the right graphic form](https://eds-240-data-viz.github.io/course-materials/lecture-slides/lecture2.1-choosing-graphic-forms-slides.html#/title-slide). Refer to this lecture when answering the sub-questions, below:**
    ...
    b.  **What task(s) do you think the author wanted to enable or message(s) he wanted to convey with these visualizations (see lecture 2.1, slide 16 for examples)? Be sure to note at least one task / message for each of the three data viz.**
        ...
        -   For the UFO shape visualization, the author wanted to enable comparison of which shapes are most commonly reported, revealing that "light" is the most frequent category by representing the first bar as lighter. For the US map, the message is to reveal geographic patterns and show where sightings are concentrated across different states. For the time-of-day visualization, the goal is to show temporal patterns and enable viewers to see when sightings peak throughout the day and week, with a plot resembling a clock.
...
21. **Describe two elements of this piece that you find visually-pleasing / easy to understand / intuitive. Why?**
    ...
    -   The color scheme is intuitive and thematic, using dark backgrounds with accent colors that convey a UFO aesthetic. The use of the UFO image as a decorative element is visually-pleasing and reinforces the theme without cluttering the information.
